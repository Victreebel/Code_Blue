...
import {
  processSelfAssignments,
  generateAmbientSpeech,
  handleComplication,
  generateSpontaneousBehaviors,
} from "./teamAI";
...
export function useGameEngine() {
  ...
  useEffect(() => {
    ...
    if (prev.hasIV !== cur.hasIV) {
      console.log(
        `%c[${t}] hasIV: ${prev.hasIV} → ${cur.hasIV}`,
        "color: #22d3ee; font-weight: bold",
      );
    }
    if (prev.hasIO !== cur.hasIO) {
      console.log(
        `%c[${t}] hasIO: ${prev.hasIO} → ${cur.hasIO}`,
        "color: #22d3ee; font-weight: bold",
      );
    }
    if (prev.rhythm !== cur.rhythm) {
      console.log(
        `%c[${t}] rhythm: ${prev.rhythm} → ${cur.rhythm}`,
        "color: #f59e0b; font-weight: bold",
      );
    }
    if (prev.cprInProgress !== cur.cprInProgress) {
      console.log(
        `%c[${t}] cprInProgress: ${prev.cprInProgress} → ${cur.cprInProgress}`,
        "color: #a78bfa; font-weight: bold",
      );
    }
    if (prev.hasAdvancedAirway !== cur.hasAdvancedAirway) {
      console.log(
        `%c[${t}] hasAdvancedAirway: ${prev.hasAdvancedAirway} → ${cur.hasAdvancedAirway}`,
        "color: #34d399; font-weight: bold",
      );
    }
    if (prev.hasPulse !== cur.hasPulse) {
      console.log(
        `%c[${t}] hasPulse: ${prev.hasPulse} → ${cur.hasPulse}`,
        "color: #f43f5e; font-weight: bold",
      );
    }
    if (prev.rolesConfirmed !== cur.rolesConfirmed) {
      console.log(
        `%c[${t}] rolesConfirmed: ${prev.rolesConfirmed} → ${cur.rolesConfirmed}`,
        "color: #94a3b8",
      );
    }
    if (prev.defibCharged !== cur.defibCharged) {
      console.log(
        `%c[${t}] defibCharged: ${prev.defibCharged} → ${cur.defibCharged}`,
    ...
    if (prev.medications.length !== cur.medications.length) {
      ...
      console.log(
        `%c[${t}] medication applied: ${newMed.type} ${newMed.dose}`,
    ...
    if (Math.abs(prev.bp.systolic - cur.bp.systolic) >= 5) {
      console.log(
        `[${t}] bp: ${prev.bp.systolic}/${prev.bp.diastolic} → ${cur.bp.systolic}/${cur.bp.diastolic}`,
      );
    }
    ...
    if (prev.identifiedCause !== cur.identifiedCause) {
      console.log(
        `%c[${t}] identifiedCause: ${prev.identifiedCause ?? "none"} → ${cur.identifiedCause ?? "none"}`,
        "color: #e879f9; font-weight: bold",
      );
    }
    if (prev.treatedCause !== cur.treatedCause) {
      console.log(
        `%c[${t}] treatedCause: ${prev.treatedCause} → ${cur.treatedCause}`,
        "color: #e879f9; font-weight: bold",
      );
    }
      ...
      for (let i = 0; i < state.scenario.scheduledEvents.length; i++) {
        ...
        if (!firedEventsRef.current.has(i) && now >= evt.time) {
          ...
          if (evt.type === "iv_lost") {
            ...
            if (leaver) {
              dispatch({
                type: "COMPLICATION_STAFF_LEAVES",
                memberId: leaver.id,
              });
            }
            ...
            const otherRhythms = allRhythms.filter(
              (r) => r !== state.patient.rhythm,
            );
            const newRhythm =
              otherRhythms[Math.floor(Math.random() * otherRhythms.length)];
    ...
    if (now - aiCheckRef.current > 3) {
      ...
      for (const evt of spontaneous) {
        ...
        if (evt.eventType !== "initiative") {
          ...
          const logMsg =
            evt.eventType === "clarification"
              ? `${name} asks for clarification`
              : evt.eventType === "distraction"
                ? `${name} is distracted`
                : evt.eventType === "wrong_task"
                  ? `${name} performed wrong task`
                  : evt.eventType === "duplicate"
                    ? `${name} duplicated a task`
                    : `${name}: behavioral event`;
          dispatch({
            type: "FIRE_EVENT",
            event: { time: state.clock, type: "cpr_fatigue", fired: true },
          });
        }
      }
    }
  ...
  const actions = {
    startGame,
    beginCode,
    assignRole: (memberId: string, role: TeamRole) =>
      dispatch({ type: "ASSIGN_ROLE", memberId, role }),
    confirmRole: (memberId: string) =>
      dispatch({ type: "CONFIRM_ROLE", memberId }),
    ...
    orderMedication: (med: MedicationType, dose: string) =>
      dispatch({ type: "ORDER_MEDICATION", medication: med, dose }),
    orderAirway: (advanced: boolean) =>
      dispatch({ type: "ORDER_AIRWAY", advanced }),
    orderIVAccess: (io: boolean) => dispatch({ type: "ORDER_IV_ACCESS", io }),
    identifyCause: (cause: ReversibleCause) =>
      dispatch({ type: "IDENTIFY_CAUSE", cause }),
    treatCause: () => dispatch({ type: "TREAT_CAUSE" }),
    kickMember: (memberId: string) =>
      dispatch({ type: "KICK_MEMBER", memberId }),
    chargeDefib: () => dispatch({ type: "CHARGE_DEFIB" }),
    requestCompressorSwitch: () =>
      dispatch({ type: "REQUEST_COMPRESSOR_SWITCH" }),
